...
```python
# 檔名: main.py
# 智慧文件處理與範本管理系統 - Streamlit 版本 (手機優化版)
...
def setup_page_config():
    ...
    st.set_page_config(
        ...
        page_icon="📄",
        layout="centered",  # 移動端使用centered布局更好
        initial_sidebar_state="collapsed",  # 移動端預設收起側邊欄
        menu_items={
            'Get Help': None,
            'Report a bug': None,
            'About': "📄 PDF文件比對與範本管理系統 v3.0 - 移動端優化版"
        }
    )
    
    hide_streamlit_style = """
    <style>
    /* 隱藏 Streamlit 自動生成的導覽 */
